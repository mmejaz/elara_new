...
/** Server-side paginated + searchable module list — for the Module Builder table. */
export function useModulesPaginated(params: ServerTableParams) {
  return useQuery({
    queryKey: ['modules-paginated', params],
    queryFn: async (): Promise<Paginated<Module>> => {
      const { data } = await apiClient.get('/modules/paginated', { params })
      return data
    },
...
export function useCreateModule() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['modules-paginated'] })
