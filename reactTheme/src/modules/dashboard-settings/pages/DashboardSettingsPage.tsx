import { AppstoreOutlined, DeleteOutlined, ExpandOutlined, PlusOutlined, SaveOutlined } from '@ant-design/icons'
import { Button, Card, Col, Empty, Modal, Popconfirm, Row, Space, Spin, Switch, Tooltip, Typography, theme } from 'antd'
import { useEffect, useMemo, useState } from 'react'
import { useTranslation } from 'react-i18next'
import PageHeader from '../../../components/PageHeader'
import { useAppSelector } from '../../../store/hooks'
import { toast } from '../../../utils/toast'
import WidgetPreview from '../../dashboard/widgets/WidgetPreview'
...
function DashboardSettingsPage() {
  ...
  // Widget being previewed live in a modal (null = closed).
  ...
  const previewWidget = data?.widgets.find((w) => w.key === previewKey) ?? null
  ...
  return (
    <Space direction="vertical" size={14} className="w-full">
            ...
            >
              {activeWidget ? (
                (() => {
                  ...
                  const visibleCount = enabledCount(activeWidget.key)
                  return (
                    <div className="flex flex-col gap-3 p-1.5">
                      {widgetHidden && (
                        <div
                          className="rounded-lg px-3 py-2 text-[12px]"
                          style={{ background: token.colorFillSecondary, color: token.colorTextTertiary }}
                        >
                      ...
                      {/* Live preview — the widget exactly as it renders on the dashboard. */}
                      <div>
                        <div className="mb-1.5 flex items-center justify-between px-0.5">
                          <Text type="secondary" className="!text-[11px] font-semibold uppercase tracking-wider">
                            {t('dashboardSettings.livePreview', { defaultValue: 'Live preview' })}
                          </Text>
                          <Tooltip title={t('dashboardSettings.expand', { defaultValue: 'Expand' })}>
                            <Button
                              type="text"
                              size="small"
                              icon={<ExpandOutlined />}
                              onClick={() => setPreviewKey(activeWidget.key)}
                            />
                          </Tooltip>
                        </div>
                        <div
                          ...
                          style={{
                            ...
                            opacity: widgetHidden ? 0.6 : 1,
                          }}
                        >
                          <WidgetPreview widget={activeWidget} />
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="mb-1.5 flex items-center justify-between px-0.5">
                          <Text type="secondary" className="!text-[11px] font-semibold uppercase tracking-wider">
                            {t('dashboardSettings.visibleToRoles', { defaultValue: 'Visible to roles' })}
                          </Text>
                          <Text
                            type="secondary"
                            className="!text-[11px] font-medium tabular-nums"
                            style={{ color: visibleCount === total ? '#16a34a' : undefined }}
                          >
                            {visibleCount}/{total}
                          </Text>
                        </div>
      ...
      {/* Live preview of a single widget, rendered exactly as it appears on the
          dashboard (resolved through the widget registry). */}
      <Modal
        ...
        title={
          previewWidget && (
            <Space size={8}>
              <span style={{ color: primaryColor }}>{widgetIcon(previewWidget)}</span>
              <span>{label(previewWidget)}</span>
            </Space>
          )
        }
      >
