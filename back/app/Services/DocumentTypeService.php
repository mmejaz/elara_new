<?php

namespace App\Services;

use App\Http\Resources\DocumentTypeResource;
use App\Models\DocumentType;
use Illuminate\Contracts\Pagination\LengthAwarePaginator;
use Illuminate\Support\Facades\DB;

class DocumentTypeService
{
    /** Columns that may be sorted from the client (whitelist). */
    private array $sortable = ['name', 'created_at', 'id'];

    public function paginate(array $params): LengthAwarePaginator
    {
        $query = DocumentType::query();

        if (! empty($params['search'])) {
            $query->where('name', 'like', '%' . $params['search'] . '%');
        }

        $sortBy = in_array($params['sort_by'] ?? '', $this->sortable, true)
            ? $params['sort_by']
            : 'created_at';
        $sortDir = ($params['sort_dir'] ?? 'desc') === 'asc' ? 'asc' : 'desc';

        return $query
            ->orderBy($sortBy, $sortDir)
            ->paginate((int) ($params['per_page'] ?? 15));
    }

    public function create(array $data): DocumentTypeResource
    {
        return DB::transaction(function () use ($data) {
            $record = DocumentType::create(['name' => $data['name']]);

            return new DocumentTypeResource($record);
        });
    }

    public function update(DocumentType $documentType, array $data): DocumentTypeResource
    {
        return DB::transaction(function () use ($documentType, $data) {
            $documentType->update(['name' => $data['name']]);

            return new DocumentTypeResource($documentType);
        });
    }

    public function delete(DocumentType $documentType): void
    {
        DB::transaction(fn () => $documentType->delete());
    }
}
